...
import { EntryPoint, EntryPoint__factory, IEntryPoint, IERC20, SampleAcct__factory, TestAggregatedAccount__factory } from '../typechain'
...
export function getAccountDeployer (entryPoint: string, owner: string): BytesLike {
  const accountCtr = new SampleAcct__factory(ethers.provider.getSigner()).getDeployTransaction(entryPoint, owner).data!
