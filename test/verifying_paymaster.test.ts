...
describe('EntryPoint with VerifyingPaymaster', function () {
  ...
  describe('#validatePaymasterUserOp', () => {
    it('should reject on no signature', async () => {
      ...
      await expect(entryPointStatic.callStatic.simulateValidation(userOp, '0x')).to.be.revertedWith('invalid signature length in paymasterData')
    })
    ...
    it('should reject on invalid signature', async () => {
      ...
      await expect(entryPointStatic.callStatic.simulateValidation(userOp, '0x')).to.be.revertedWith('ECDSA: invalid signature')
    })
