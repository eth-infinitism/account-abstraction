...
describe('SampleAcct', function () {
  ...
  it('owner should be able to call transfer', async () => {
    const account = await new SampleAcct__factory(ethers.provider.getSigner()).deploy(entryPoint, accounts[0])
  ...
  describe('#validateUserOp', () => {
    ...
    before(async () => {
      ...
      account = await new SampleAcct__factory(await ethers.getSigner(entryPoint)).deploy(entryPoint, accountOwner.address)
  ...
  context('SampleAcctDeployer', () => {
    it('sanity: check deployer', async () => {
      const ownerAddr = createAddress()
      const deployer = await new SampleAcctDeployer__factory(ethersSigner).deploy()
