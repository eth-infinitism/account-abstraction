...
describe("Gnosis Proxy", function () {
  ...
  it("#getCurrentEIP4337Manager", async () => {
    ...
    const tempManager = await new EIP4337Manager__factory(ethersSigner).deploy(
      AddressZero,
    );
  ...
  it("should revert with reason", async function () {
    ...
    const safe_execFailTxCallData = manager.interface.encodeFunctionData(
      "executeAndRevert",
      [counter.address, 0, counter_countFailCallData, 0],
    );
  ...
  it("should create account", async function () {
    ...
    counterfactualAddress = await accountFactory.callStatic.getAddress(
      ownerAddress,
      123,
    );
  ...
  it("should validate ERC1271 signatures", async function () {
    ...
    expect(await safe.isValidSignature(dataHash, badSig)).to.be.not.eq(
      "0x1626ba7e",
    );
  });
  ...
  context("#replaceEIP4337", () => {
    ...
    before(async () => {
      ...
      signature = hexConcat([hexZeroPad(ownerAddress, 32), HashZero, "0x01"]);
      newEntryPoint = await new EntryPoint__factory(ethersSigner).deploy();

      newManager = await new EIP4337Manager__factory(ethersSigner).deploy(
    ...
    it("should reject to replace if wrong old manager", async () => {
      const replaceManagerCallData = manager.interface.encodeFunctionData(
    ...
    it("should replace manager", async function () {
      ...
      expect(await proxySafe.isModuleEnabled(entryPoint.address)).to.equal(
        true,
      );
      ...
      expect(await proxySafe.isModuleEnabled(newEntryPoint.address)).to.equal(
        true,
      );
