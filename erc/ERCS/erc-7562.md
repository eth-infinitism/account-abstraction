...
* **[STO-041]** A contract whose storage is associated with any entity of a `UserOperation` may not be a "sender" of another UserOperation in the mempool.
...
* **[EREP-010]** For each `paymaster`, the mempool must maintain the total gas `UserOperations` using this `paymaster` may consume.
    ...
    * **[EREP-011]** a staked paymaster is required to have only TODO: allow "fractional reserve" based on reputation?
...
Below are examples of possible attacks that were considered and a reference to the above rule that prevents them.
...
* Mitigations
    * A **staked factory:**
        * Can only run one iteration of this attack, since after invalidating a single bundle creation its reputation drops and another staked factory has to be used.
