...
This method will be called by the entryPoint with the current UserOperation.
...
* If the calldata starts with the methodsig `IAccountExecute.executeUserOperation`, then the EntryPoint must build a calldata that contains the entire UserOperation and hash and call the account using that calldata.
...
2. **factory** and **dactoryData** - either both exist, or none
3. paymaster fields either all 4 exist, or none.
