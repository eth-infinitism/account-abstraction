...
* If the calldata starts with the methodsig `IAccountExecute.executeUserOp`, then the EntryPoint must build a calldata that contains the entire UserOperation and hash and call the account using that calldata.
