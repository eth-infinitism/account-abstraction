...
import { EntryPoint, EntryPoint__factory, SampleAcct__factory } from '../typechain'
...
import { SampleAcctInterface } from '../typechain/contracts/samples/SampleAcct'
...
export class GasChecker {
  ...
  accountInterface: SampleAcctInterface

  constructor () {
    this.accountOwner = createAccountOwner()
    this.accountInterface = SampleAcct__factory.createInterface()
  ...
  accountInitCode (): string {
    return hexValue(new SampleAcct__factory(ethersSigner).getDeployTransaction(GasCheckCollector.inst.entryPoint.address, this.accountOwner.address).data!)
  }
