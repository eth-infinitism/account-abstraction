...
contract GnosisSafeAccountFactory {
    ...
    function createAccount(address owner, uint256 salt) public returns (address) {
        ...
        return address(proxyFactory.createProxyWithNonce(safeSingleton, getInitializer(owner), salt));
    }
    ...
    function getInitializer(address owner) internal view returns (bytes memory) {
        ...
        bytes memory setup4337Modules = abi.encodeCall(EIP4337Manager.setup4337Modules, (eip4337Manager));
