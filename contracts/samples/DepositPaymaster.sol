...
contract DepositPaymaster is BasePaymaster {
    ...
    function getTokenValueOfEth(IERC20 token, uint ethBought) internal view virtual returns (uint requiredTokens) {
    ...
    external view override returns (bytes memory context) {
        ...
        uint maxTokenCost = getTokenValueOfEth(token, maxCost);
