...
(async () => {
  ...
  const info = await entryPoint.getDepositInfo(myAddress)
  const currentDeposit = info.amount.toString()
  console.log('current deposit=', currentDeposit)

  if (info.amount.lte(parseEther('0.01'))) {
  ...
  console.log('current counter=', await testCounter.counters(myAddress), 'balance=', prebalance, 'stake=', currentDeposit)
