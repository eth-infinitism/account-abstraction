...
When a wallet is called through an aggregator, it exposes instead:
```solidity
interface IWallet {
...
```
* **validateUserOp()** the same as IWallet's method, but with an extra `aggregator` parameter. The wallet MUST verify that this is the same as getAggregator
...
* validateUserOp MAY completely ignore the signature field
...
* Before the first validation loop, it should scan the aggregators list and call its `validateSignatures(UserOperation[], aggregatedSignature)`, with its respective userOps, and revert if the validation fails
...
* If the wallet is part of an aggregated wallet loop, use the `validateUserOp` from IAggregatedWallet, to pass also the aggregator address
