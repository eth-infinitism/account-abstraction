...
* it is called from simulateValidation() returns the aggregator's address.
...
4. The third (validateUserOpSignature) call doesn't access _mutable state_ of any contract except the aggregator AND wallet itself.
5. Any `CALL` or `CALLCODE` during validation has `value=0`, except for the transfer from the wallet to the entry point.
6. No `CALL`, `DELEGATECALL`, `CALLCODE`, `STATICCALL` results in an out-of-gas revert.
