...
In addition, a UserOperation is only allowed to reference data related to this sender address: Multiple UserOperations should not reference the same storage, so that it is impossible to invalidate a large number of UserOperations with a single state change.
...
2. The first (validateUserOp) call is allowed to access storage slot of the wallet itself (see Storage access by Slots, below), 
...
3. The second (validatePaymasterUserOp), the paymaster may access storage slots of itself AND of the wallet, above.
   A bundler MAY whitelist specific paymaster addresses, and not enforce this storage access limitation.
4. The third (validateUserOpSignature) view-call, the aggregator is allowed to access storage slots of itself and of the wallet. 
...
7. No referenced address (`CALL`, `DELEGATECALL`, `CALLCODE`, `STATICCALL`, `EXTCODESIZE`, `EXTCODEHASH` to addresses with `EXTCODESIZE=0`.
...
A contract `A` may reference storage of
...
4. Slots of type `keccak256(X || OWN)` on any other address, where OWN is some slot of the previous (third) type 
