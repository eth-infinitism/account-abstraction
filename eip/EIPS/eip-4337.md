...
* in case of failure, MUST return an `error` result object, with `code` and `message`. The error code and message SHOULD be set as follows:
  ...
  * **code: -32501** - transaction rejected because paymaster is throttled/banned
  * **code: -32502** - transaction rejected because wallet specified unsupported signature aggregator
    * The `data` field SHOULD contain a `aggregator` value
  * **code: -32503** - transaction rejected because of opcode validation
  * **code: -32504** - UserOperation expired: either wallet or paymaster returned an `deadline` value in the past.
...
In case the UserOperation is not yet mined, return `null`
...
**Return value**:
TransactionReceipt object, or `null` in case the UserOperation is not yet mined.
