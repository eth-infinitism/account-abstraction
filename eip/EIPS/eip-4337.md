...
simulation receives a list of aggregators that are called directly to validate the signature.
Blocking userOp with a banned aggregator is done during [Simulation](#simulation)

The call to simulateValidation reverts with "UnverifiedSignatureAggregator" in case the aggregator is not in the list of allowed aggregators.
The error returns the aggregator's address, so the bundler may still validate the signature 
(by calling aggregator.validateUserOpSignature or using an off-chain library), and accept this UserOperation
In such case, the bundler MAY also add this aggregator to the list of allowedAggregators for the next simulation calls. 
