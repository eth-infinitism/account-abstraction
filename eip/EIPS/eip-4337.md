...
### debug Namespace
This api should only be available on testing mode. In production, any `debug_*` rpc calls should be blocked.
...
`aggregator` - (optional) the aggregator used by the sender account.
...
`array` - Array of reputation objects, each with the following fields:
...
Returns an array of reputation objects, each with the fields described above in `debug_bundler_setReputation` with the extra field:

`status` - (string) The serving status of the given address in the bundler . One of: `'ok' | 'throttled' | 'banned'.`
