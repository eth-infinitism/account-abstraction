...
function simulateValidation
    ...
    external returns (uint256 preOpGas, uint256 prefund, uint256 deadlne address actualAggregator, bytes memory sigForUserOp, bytes memory sigForAggregation, bytes memory offChainSigInfo) {
...
* The return value `deadline` is either zero, or the last timestamp this request is deemed valid.
