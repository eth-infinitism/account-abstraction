...
First, a paymaster must issue `addStake()` to lock some eth for a period of time, unless it is exempt due to not accessing global storage - see [reputation, throttling and banning section](#reputation-scoring-and-throttlingbanning-for-global-entities) for details. Note that the amount staked (and unstake delay time) are not defined on-chain, and MUST be
...
Maliciously crafted paymasters _can_ DoS the system. To prevent this, we use a paymaster reputation system; see the [reputation, throttling and banning section](#reputation-scoring-and-throttlingbanning-for-global-entities) for details.
...
- The paymaster is either the zero address or is a contract which (i) currently has nonempty code on chain, (ii) has a sufficient deposit to pay for the UserOperation, and (iii) is not currently banned. At this point, also record whether the paymaster is staked, in case the later simulation indicates that it needs to be. The paymaster must be staked if it accesses global storage not directly associated with the account - see [reputation, throttling and banning section](#reputation-scoring-and-throttlingbanning-for-global-entities) for details.
...
There are 3 special contracts that interact with the account: the deployer (initCode) that deploys the contract, the paymaster that can pay for the gas, and signature aggregator (described later)
...
calling:
1. create account, `initCode` is present
...
3. Storage access is limited as follows:
   1. self storage (of deployer/paymaster, respectively) is allowed, but only if self entity is staked
   2. account storage access is allowed (see Storage access by Slots, below),
   3. in any case, may not use storage used by another UserOp `sender` in the same bundle (that is, paymaster and deployer are not allowed as senders)
...
But "global" entities - paymasters, deployers and aggregators are accessed by multiple UserOperations, and thus might invalidate multiple previously-valid UserOperations.
...
(it is allowed to use storage specific to the account, such as mapping keyed by account address)
...
In the following specification, "entity" is either address that is explicitly referenced by the UserOperation: sender, deployer, paymaster and aggregator.
Clients maintain two mappings with a value for each entity:

* `opsSeen: Map[Address, int]`
* `opsIncluded: Map[Address, int]`

If an entity doesn't use storage at all, or only reference the "sender" storage (see [Storage access by slots](#storage-access-by-slots)), then it is considered "OK", without using the rules below.

When the client learns of a new entity, it sets `opsSeen[paymaster] = 0` and `opsIncluded[paymaster] = 0` .
