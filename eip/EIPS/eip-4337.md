...
* **UserOperation** - a structure that describes a transaction to be sent on behalf of a user. To avoid confusion, it is named "UserOperation" instead of "transaction."
...
An account signify it uses signature aggregation returning its address from `validateUserOp`.
...
6. `EXTCODEHASH`, `EXTCODELENGTH`, `EXTCODECOPY` may not access the EntryPoint contract address or an address with no deployed code.
