...
In case the UserOp creates a new wallet, the aggregator is returned by the `simulateValidationWithAggregators()` call.

To use an off-chain optimized implementation, the bundler MAY:
* call `simulateValidationWithAggregators()`, without specifying the aggregator in the list of validateAggregators
...
Before accepting a `UserOperation`, bundlers should use an RPC method to locally call the `simulateValidation` (or `simulateValdiationWitAggregators`) function of the entry point, to verify that the signature is correct and the operation actually pays fees; see the [Simulation section below](#simulation) for details.
Note that the `simulateValidation()` is a wrapper helper method around `simulateValidationWithAggregators()`, for networks where signature aggregation doesn't make sense (too expensive)
