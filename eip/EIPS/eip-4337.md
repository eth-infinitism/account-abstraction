...
* **Achieve the key goal of account abstraction**: allow users to use smart contract accounts containing arbitrary verification logic instead of EOAs as their primary account. Completely remove any need at all for users to also have EOAs (as status quo SC accounts and EIP-3074 both require)
* **Decentralization**
...
The aggregator of a given account is exposed by the `getAggregator()` method. 
In case the UserOp creates a new account, the aggregator is returned by the `simulateValidation()` call.
...
To prevent attacks involving malicious `UserOperation` objects listing other users' accounts as their paymasters, the entry point contract must require a paymaster to call the entry point to lock their stake and thereby consent to being a paymaster. Unlocking stake must have a delay. The extended interface for the entry point, adding functions for paymasters to add and withdraw stake, is:
...
2. The first (validateUserOp) call does not access _mutable state_ of any contract except the account itself and its deposit in the entry point contract. _Mutable state_ definition includes both storage and balance.
...
4. The third (validateUserOpSignature) view-call doesn't access _mutable state_ of any contract except the aggregator AND account itself.
...
6. Any `CALL` or `CALLCODE` during validation has `value=0`, except for the transfer from the account to the entry point.
...
Since the account is allowed to access its own entry point deposit in order to top it up when needed, the client must know the storage slot in order to whitelist it. The entry point therefore implements the following view function:
...
NOTE: In order for the account to determine the "counterfactual" address of the account (prior its creation), 
