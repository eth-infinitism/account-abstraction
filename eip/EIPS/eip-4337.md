...
* **UserOperation** - a structure that describes a transaction to be sent on behalf of a user. To avoid confusion, it is named named "transaction".
...
* **Aggregator** - a helper contract trusted by wallets to validate an aggregated signature.
...
* MUST pay the entryPoint (caller) at least the "missingWalletFunds" 
...
```
If the wallet supports aggregation, this method should return the aggress of the aggregator.
In this case, the `validateUserOp` MUST validate that the signature's length is zero.
