...
* If a wallet uses an aggregator (returns it with getAggregator()), then the `simulateValidation()` calls reverts with **UnverifiedSignatureAggregator** error
...
The call to simulateValidation reverts with `UnverifiedSignatureAggregator` if the wallet uses an aggregator.
...
(In case aggregation is supported, the special UnverifiedSignatureAggregator error is used)
