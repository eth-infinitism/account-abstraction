...
To prevent cross-chain replay attacks, `signature` should depend on `chainid`.
...
- The sender doesn't have another `UserOperation` already present in the pool (or it replaces an existing entry with the same sender, nonce with a higher priority). Only one `UserOperation` per sender per batch allowed.
...
In order to distinguish between the two, there is a single call to NUMBER opcode (`block.number`), so the client can run "wallet validation" on the first half,
...
- Keep track of paymasters' balances, to verify it doesn't run out of funds during execution.

After creating the batch, before executing the transaction, the client should:
...
- Trace `FailedOp` events, and remove the matching ops that fail from the batch, until simulation succeeds.
