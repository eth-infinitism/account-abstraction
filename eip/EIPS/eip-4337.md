...
* If the wallet does not support signature aggregation, it MUST validate the signature is a valid signature of the `userOpHash` 
  In order to support eth_simulateValidation for unsigned UserOperation, the wallet SHOULD NOT revert on failed signature check if `tx.origin` is the zero address.
