...
function handleOps
    (UserOperation[] calldata ops, address payable beneficiary
     AggregatorInfo[] calldata aggregators, uint[] memory execOrder
    ) public

function simulateValidation
    (UserOperation calldata userOp, aggregatedSignature)
...
* MUST validate the signature is a valid signature of the `requestId`
...
* the wallet MAY pay more than this minimum, to cover future transactions
* Note that in case the request comes through an aggregator, the signature will be empty.

A Wallet that supports aggregated signature should have the interface:
```solidity
...
* The wallet is guaranteed to that the EntryPoint will only call its `validateUserOp` (with no signature) after calling the aggregator's validateSignatures
...
* validateUserOp shouldn't validate the signature (since it is verified by the aggregator),
  but it MUST verify the signature field is shorter than 63 bytes
...
* bundlers/nodes must have explicit support for each aggregator, and know the code they need to calculate its aggregated signature.
* the validateSignatures method is called by the entrypoint to let the aggregator validate the aggregate signature is valid for all UserOps.
* if the signature is invalid or doesn't match, the aggregator should revert.
* the aggregator SHOULDN'T call the getAggregator() of each wallet: the entrypoint already validated that on each userOp before calling validateSignatures
...
* Before the first validation loop, if an aggregator is specified, the entryPoint must call its `validateSignatures(UserOperation[], aggregatedSignature)`, and revert if the validation fails
...
* in case the UserOperation contains an empty signature, the entryPoint MUST call the wallet's getAggregator, and MUST revert in case the wallet doesn't implement getAggregator or it doesn't match the given **aggregator** parameter
...
A bundler SHOULD drop (and not add to the mempool) `UserOperation` that fails the validation
...
A Signature Aggregator is validate an aggregated signature of a group of UserOperations
The aggregator exposes a single method:
```
  function validateSignatures(address entryPoint, UserOp[], bytes aggregatedSignature) view;
```
The UserOps are all without signatures (either empty, or less than 64 bytes, if aggregator-wallet requires some extra info)
The aggregator should scan all the UserOps, and validate the aggregated signature on all of them.
The entryPoint should be used to calculate the userOp hash, along with the current chainId
The aggregator should revert if the signature check fails.
...
To simulate a `UserOperation` `op` validation, the client makes a view call to `simulateValidation(userop,aggregatedSgianture)`, with a "from" address set to all-zeros

e.g. makes an `eth_call` with the following params:
...
First, if the wallet specifies an aggregator using `getAggregator()`, then the client should verify it as a "bundle of one entry" 
(e.g with the userOp.signature entry cleared, and with aggregatedSignature)
If the wallet doesn't expose a getAggregator(), the aggregatedSgianture parameter should be left zero.

The simulated call performs the full validation, first validating signature using the aggregator (if needed), then 
calling both `wallet.validateUserOp` and (if specified) `paymaster.validatePaymasterUserOp`.
...
- A batch can include UserOperations of "unaggregated" wallets or wallets that use a single aggregator
  (that is, a single bundle can't include operations of multiple aggregators)
- The signatures of all aggregated wallets are aggregated into a single signature.
- Those UserOperations are then added to the bundle with their own signature cleared.
