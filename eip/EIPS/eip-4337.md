...
To prevent (both cross-chain and multiple `EntryPoint` implementations) replay attacks, the `signature` should depend on `chainid` and the `EntryPoint` address.
...
- Trace `FailedOp` events. Remove the failed op that caused the revert from the batch and drop from the mempool. Other ops from the same paymaster should be removed from the current batch, but kept in the mempool. Repeat until simulation succeeds. Note that a failed op is usually detected on single op simulation before, thus failing here is unusual.
