...
```
If the wallet supports aggregation, this method should return the address of the aggregator.
In this case, the `validateUserOp` MUST validate that the signature's length is zero.
Note: depending on aggregator type, the wallet must also expose
