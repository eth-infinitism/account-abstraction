...
function simulateValidation
    ...
    external returns (uint256 preOpGas, uint256 prefund, address aggregator)
...
* it is called by the bunder just after simulateValidation() returns the aggregator's address.
...
They MAY also use off-chain optimized versions of **validateUserOpSignature** and **aggregateSignatures**
...
If `simulateValidation()` returns a non-zero **aggregator**, the client MUST call aggregator.validateUserOperation to validate the signature.
This method also returns the value used for aggregation when the UserOp is submitted.
