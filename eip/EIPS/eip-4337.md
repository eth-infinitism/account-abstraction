...
author: Vitalik Buterin (@vbuterin), Yoav Weiss (@yoavw), Kristof Gazso (@kristofgazso), Namra Patel (@namrapatel), Dror Tirosh (@drortirosh), Shahaf Nacson (@shahafn)
...
### UserOperation gas parameters detailed:

#### callGas

The gas limit for actual callData execution

#### verificationGas

This value defines the maximum value the bunlder might spend on verifying the UserOperation. It should be enough to run both the `wallet.validateUserOp` and `paymaster.validatePaymasterUserOp`.
Note that if `initCode` exists, then the verificationGas should also accomodate for creation of the sender contract.
A node MAY drop requests if the verificationGas is too high, but SHOULD report it as an error if it was added through RPC call.

The value also used as a limit on the `postOp()` call of the paymaster, if one is used.
(and at a worst-cast might be used twice for postOp)

#### preVerificationGas

This value covers all the gas usage that can't be covered by GAS opcode checks.
Note that callGas and verificationGas are **upper limits** (user pays for actual used portion), but preVerificationGas is used as-is.

- static overhead of `handleOps()` 
    (including stipend of 21000, bundler compensation (9500) and some internal overhead)

- dynamic (per UserOp) overhead: 
  cost of putting the userOp data on chain (based in EIP-2028) (about 6000 for a small transaction) and per-userOp overhead.

#### Calculation of "max possible gas"

When validating a wallet/paymaster has enough to pay for a transaction, the max possible gas is calculated as follows:

- Is paymaster is not set:
    `preVerificationGas + callGas + verificationGas`

- If paymaster is set:    
    `preVerificationGas + callGas + verificationGas * 3`
...
5. Any `CALL`, `DELEGATECALL`, `CALLCODE`, `STATICCALL` during validation passes max gas, `gas(2**256-1)`.
6. `EXTCODEHASH` of every address accessed (by any opcode) does not change between first and second simulations of the op.
7.  If `op.initcode.length != 0` ,allow only one `CREATE2` opcode call, otherwise forbid `CREATE2`.
