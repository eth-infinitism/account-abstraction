...
* **validateUserOp()** MUST verify the `aggregator` parameter is valid and the same as `getAggregator`
...
* it is called from `simulateValidation()`.
...
Blocking userOp of banned of aggregator is done during [Simulation](#simulation)
The aggregator of a given wallet is exposed by the `getAggregator()` method. 
In case the UserOp creates a new wallet, it is returned by the `simulateValidation()` call.
...
* In case the aggregator is trusted, but doesn't support off-chain optimization, the bundler MAY  
  repeat the simulateValidation, or just call the aggregator.validateUserOpSignature to complete the validation. 

### Required entry point contract functionality

There are 2 separate entry point methods: `handleOps` and `handleAggregatedOps`
* `handleOps` handle requests that don't require any signature aggregator.
* `handleAggregatedOps` can handle a batch that contains requests of multiple aggregators (and requests without any aggregator)
* `handleAggregatedOps` performs the same logic as `handleOps`, but it must transfer the correct aggregator to each userOp, and also must call validateSignatures on each aggregator.
...
* If the **execOrder** parameter exists, execute the transaction in that order, instead of their "linear" order. It MUST execute each userOp EXACTLY once. 
...
If the call returns an error, the client rejects the `userOp`.
...
In order to distinguish between them, there is a single call to the NUMBER opcode (`block.number`), used as a delimiter between the validation functions.
...
3. The second (validatePaymasterUserOp) call does not access _mutable state_ of any contract except the paymaster itself.
