...
* in case of failure, MUST return an `error` result object, with `code` and `message`. The error code and message SHOULD be set as follows:
  ...
  * **code: -32500** - transaction rejected by entryPoint's simulateValidation.
    * The `message` field SHOULD be set to the FailedOp error message
...
Execution first performs the validation (validateUserOp, paymaster.validatePaymasterUserOp)
Note that validation is performed from the zero address, and thus validation can be done without valid signature. 

**Parameters**: same as `eth_sendUserOperation`

#### Return Value:
* **success** did the execution completed successfully (true) or reverted (false)
...
Given UserOperation without gas limits, return the needed gas limits

**Parameters**: same as `eth_sendUserOperation`
  gas limits are optional.

#### Return Values:
* **callGasLimit** did the execution completed successfully (true) or reverted (false)
* **verificationGasLimit** the total amount to be paid for this execution (including validation)
* **preVerificationGas** the gas price used to calculate gas cost (depends on the UserOp's priorityFee, maxFeePerGas and also on the network's basefee)
