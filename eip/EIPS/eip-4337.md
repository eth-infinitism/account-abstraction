...
- The paymaster is either the zero address or is a contract which (i) currently has nonempty code on chain, (ii) has registered and staked, (iii) has a sufficient deposit to pay for the UserOperation, and (iv) has not been banned.
...
2. The first call does not access _mutable state_ of any contract except the wallet itself and its deposit in the entry point contract.
