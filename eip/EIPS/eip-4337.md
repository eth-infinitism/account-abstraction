...
| `preVerificationGas` | `uint256` | The amount of gas to compensate the bundler for this UserOperation that cannot be directly on-chain by EntryPoint's validation. To know more, [read here](#preverificationgas-field). |
| `maxFeePerGas` | `uint256` | Maximum fee per gas (similar to EIP 1559 `max_fee_per_gas`) |
| `maxPriorityFeePerGas` | `uint256` | Maximum priority fee per gas (similar to EIP 1559 `max_priority_fee_per_gas`) |
| `paymasterAndData` | `bytes` | Address of paymaster sponsoring the transaction, followed by extra data to send to the paymaster. Field is empty for self-sponsored transaction |
...
* If an account uses an aggregator (returns it with getAggregator()), then its address is returned by `simulateValidation()` reverting with `SimulationResultWithAggregation` instead of `SimulationResult`
...
### preVerificationGas field

`preVerificationGas` is the amount of gas that must be compensated to the bundler for the computation that is performed before verification of the `userOp` starts. This cost constitutes of gas required to send `userOperation` to the blockchain, fixed transaction cost (the 21000 gas stipend), and the overhead calculations made by the EntryPoint contract before the verification starts.

It is difficult to calculate the gas required to send `userOperation` to the blockchain and gas required to do the overhead calculations by the EntryPoint contract on-chain.

The amount of gas required to send `userOperation` is variable (and non calculable on-chain) because it constitutes of a fixed cost to send variables with pre-determined length (`sender`, `nonce`, `callGasLimit`, `verificationGasLimit`, `maxFeePerGas` and `maxPriorityFeePerGas`) and variable cost to send variables with dynamic length (`initCode`, `calldata`, `paymasterAndData`, and `signature`).

The another variable gas cost is the overhead calculations made by the EntryPoint contract. It varies with the size of bundle that is sent to the entryPoint. For example, the 21000 stipend should be split among all the user opertations sent in a single bundle.
...
During `simulateValidation`, this aggregator is returned (in the `SimulationResultWithAggregation`)
