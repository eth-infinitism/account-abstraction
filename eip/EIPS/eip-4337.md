...
An Aggregator is used to aggregate all UserOperations signatures into a single value.
The aggregator exposes a single method:
```
  function validateSignatures(UserOp[], bytes aggregatedSignature) view;
```
The aggregator should scan all UserOp with empty signatures (ignoring UserOps with signature), and validate the aggregated signature on all of them.
It should revert if the signature fails.
...
First, if the wallet specifies an aggregator using `getAggregator()`, it is used to validate the signature.
