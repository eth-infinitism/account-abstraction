...
validated by nodes during the UserOperation validation phase. The staked value MUST be above 1000 USD in native tokens (and SHOULD be larger, to compensate for native token price fluctuations)
...
The stake is expected to be above PAYMASTER_STAKE_VALUE (1000 USD in native token)
