...
The result `SHOULD` be set to the **requestId** if and only if the request passed simulation and was accepted in the client's User Operation pool. If the validation, simulation, or User Operation pool inclusion fails, `result` `SHOULD NOT` be returned. Rather, the client `SHOULD` return the failure reason.
...
#### Return value:
* If the UserOperation is valid, the client MUST return the calculated **requestId** for it
* in case of failure, MUST return an `error` result object, with `code` and `message`. The error code and message SHOULD be set as follows:
  ...
  * **code: -32500** - transaction rejected by entryPoint's simulateValidation.
    * The `message` field SHOULD be set to the FailOp error message
    * The `data` field SHOULD contain a `paymaster` value, if set by this FailedOp
  * **code: -32501** - transaction rejected because of opcode validation
  * **code: -32502** - UserOperation expired: either wallet or paymaster returned an `deadline` value in the past.
  ...
  * **code: -32503** - transaction rejected because paymaster (or signature aggregator) is throttled/banned
  * **code: -32504** - transaction rejected because paymaster (or signature aggregator) stake or unstake-delay is too low
    * The `data` field SHOULD contain a `paymaster` or `aggregator` value, depending on the failed entity
    * The `data` field SHOULD contain a `minimumStake` and `minumumUnstakeDelay`
  * **code: -32505** - transaction rejected because wallet specified unsupported signature aggregator
...
Simulate the validation of the user operation.
Simulation involves running the verification (validateUserOp, paymaster.validatePaymasterUserOp and aggregator.validateUserOpSignature)
but not the actual execution of the UserOp.

**Parameters**: just like `eth_sendUserOperation`

**Return Value**:
* **preOpGas** gas used by validation (including preVerificationGas)
* **prefund** amount the wallet need to pre-pay (above preOpGas and callGasLimit)
* **deadline** if set, until what time this userOp is valid
* **paymasterInfo** if set, current paymasterStake and paymasterUnstakeDelay
* **aggregator** if set, the signature-aggregator used by the wallet.

**Error Codes:**: just like `eth_sendUserOperation`

### * eth_callUserOperation

Execute (in-memory) the UserOperation in a bundle of its own.

**Parameters**: just like `eth_sendUserOperation`

#### Return Value:
* **success** did the execution completed successfully (true) or reverted.
* **actualGasCost** the total amount to be paid for this execution (including validation)
* **actualGasPrice** the gas price used to calculate gas cost (depends on the UserOp's priorityFee, maxFeePerGas and also on the network's basefee)
* **reason** in case the UserOperation reverted, this is the revert reason.

#### Error Codes: 
Just like `eth_sendUserOperation`

### * eth_getUserOperationTransactionByHash

Return a transaction information based on a UserOperation hash (requestId) returned by `eth_sendUserOperation`

An equivalent of `eth_getTransactionByHash`

**Parameters**
* **hash** a requestId value returned by `eth_sendUserOperation`

**Return value**:
Transaction object (note that the returned transaction object does not contain the requestId itself)
In case the UserOperation is not yet included in a block, return `null`

### * eth_getUserOperationReceipt

Return a transaction receipt based on a UserOperation hash (requestId) returned by `eth_sendUserOperation`

An equivalent of `eth_getTransactionReceipt`

**Parameters**
* **hash** a requestId value returned by `eth_sendUserOperation`

**Return value**:
TransactionReceipt object, or `null` in case the UserOperation is not yet included in a block.
